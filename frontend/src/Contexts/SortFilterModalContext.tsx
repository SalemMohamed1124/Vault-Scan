...
export const SortFilterModalContext =
  createContext<SortFilterContextProps | null>(null);
...
export function useSortFilterState<TData>({
    ...
    if (isOpen) {
      ...
      currentFilters.forEach((f) => {
        if (Array.isArray(f.value))
          newFilters[f.id] = new Set(f.value.map(String));
      });
  ...
  const applyChanges = () => {
    ...
    Object.entries(filters).forEach(([id, vals]) => {
      table
        .getColumn(id)
        ?.setFilterValue(vals.size ? Array.from(vals) : undefined);
    });
    onClose();
  };
...
// --- Provider (used for mobile sort/filter modal) ---
export function SortFilterModalProvider<TData>(props: {
  ...
  return (
    <SortFilterModalContext.Provider value={value}>
      {props.children}
    </SortFilterModalContext.Provider>
  );
}
...
export const useSortFilter = () => {
  ...
  if (!context)
    throw new Error(
      "useSortFilterContext must be used within Provider"
    );
  return context;
};

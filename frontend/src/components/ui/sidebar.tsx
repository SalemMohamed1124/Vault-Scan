...
function SidebarProvider({
  ...
  const toggleSidebar = React.useCallback(() => {
    ...
    // Toggle pinned state when manually clicking
    setIsPinned((prev) => !prev);
    return setOpen((open) => !open);
  }, [isMobile, setOpen, setOpenMobile]);

  // Ref to hold the latest toggleSidebar for stable keyboard listener
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-sidebar text-sidebar-foreground w-(--sidebar-width) p-0 [&>button]:hidden text-sidebar-foreground border-sidebar-border"
          style={
            {
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          overlay &&
            (variant === "floating" || variant === "inset"
              ? "group-data-[state=expanded]:w-[calc(var(--sidebar-width-icon)+(--spacing(4)))]"
      ...
      <div
        ...
        className={cn(
          "fixed top-[var(--header-height,2.5rem)] bottom-0 z-10 hidden h-[calc(100svh-var(--header-height,2.5rem))] w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear md:flex",
          side === "left"

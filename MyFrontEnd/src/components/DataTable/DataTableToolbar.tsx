...
export function DataTableToolbar<TData>({
  ...
  const showFilter = toolbar.filter && filterColumns && filterColumns.length > 0;
  const showExport = toolbar.export;
  const showViewOptions = toolbar.viewOptions;

  if (!showSearch && !showFilter && !showExport && !showViewOptions && !extraActions && !isMobile) return null;
        ...
        <div className="flex flex-col gap-2 w-full">
          <div className="flex flex-col gap-2 w-full ">
            ...
            <div className="flex gap-2 w-full *:flex-1 flex-row flex-wrap">
              {showExport && <TableExporter table={table} fileName={tableName} />}
          ...
          {showSearch && <DataTableSearchColumn table={table} searchColumn={searchColumn} />}
        </div>
      ) : (
        <div className="flex items-center space-x-4 w-full">
          {showSearch && <DataTableSearchColumn table={table} searchColumn={searchColumn} />}
          {showFilter && !cardsLayout && <DataTableFilterBar table={table} columns={filterColumns} />}
          <div className="ml-auto flex items-center space-x-4">
            ...
            {showViewOptions && !cardsLayout && <DataTableViewOptions table={table} />}
            {showExport && !cardsLayout && <TableExporter table={table} fileName={tableName} />}

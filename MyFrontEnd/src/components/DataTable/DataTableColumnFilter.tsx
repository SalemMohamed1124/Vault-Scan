...
export function DataTableColumnFilter<TData>({ table, columnId }: DataTableColumnFilterProps<TData>) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="sm" className="gap-2 capitalize font-semibold flex flex-wrap items-center h-9">
          <CirclePlus className="h-4 w-4 opacity-50" />
          {columnId}
          {selectedValues.map((value) => (
            <Badge key={value} variant="secondary" className="ml-1 text-xs">
              {value}
            </Badge>
          ))}
        </Button>
      ...
      {selectedValues.length > 0 && (
        <Tooltip>
          <TooltipTrigger asChild>
            <Button size="icon" variant="ghost" className="h-6 w-6 p-0" onClick={clearFilter}>
              <X className="h-3 w-3" />
            </Button>
          </TooltipTrigger>
      ...
      <DropdownMenuContent align="start" className="w-56 max-h-[300px] ">
        <DropdownMenuLabel className="capitalize">Filter by {columnId}</DropdownMenuLabel>
        <DropdownMenuSeparator />
